...
- 2026-07-26: 修复 TTS 重叠：AudioManager.speak 从"排队补播"改为"最新优先打断"——新语音到来时 cancel 旧语音并丢弃未播项，cancel 后延迟 60ms 再 speak 规避 Chrome 吞语音问题；clear()/setEnabled(false) 同步清理待播定时器。Playwright 假 synth 验证：连读 5 个字母仅首个与最新的播出、中间丢弃；reset 中断待播；真实链路 5 骨牌连锁无 console error。
